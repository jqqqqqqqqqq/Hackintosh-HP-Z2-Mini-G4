 ...
 *
 * Disassembly of ssdt1.aml, Tue Aug  4 03:47:23 2020
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00001B60 (7008)
 *     Revision         0x02
 *     Checksum         0x10
 *     OEM ID           "CpuRef"
...
DefinitionBlock ("", "SSDT", 2, "CpuRef", "CpuSsdt", 0x00003000)
{
    ...
    {
        Name (CTPC, Zero)
        OperationRegion (PNVS, SystemMemory, 0xB3DD3000, 0x0072)
    ...
    Scope (\_SB.PR00)
    {
        ...
        Method (GCAP, 1, Serialized)
        {
            ...
            If ((((CFGD & One) || (CFGD & 0x00400000)) && !(SDTL & 0x0100)))
            {
                SDTL |= 0x0100
            ...
            If ((((\_SB.CFGD & 0x02000000) && (\_SB.CFGD & 0x00400000)) && (\_SB.OBIN != Zero)))
            {
                If ((\_SB.OSCP & 0x1000))
                {
                    IOB2 = ITBS
    ...
    Scope (\_SB.PR01)
    {
        ...
        {
            If ((((CFGD & One) || (CFGD & 0x00400000)) && !(SDTL & 0x0200)))
            {
                SDTL |= 0x0200
    ...
    Scope (\_SB)
    {
        If ((TXTE == One))
        {
            Device (ACM)
            {
                ...
                Name (_CID, "INT_WHL_SINIT2")  // _CID: Compatible ID
            }
        }
    }
}
